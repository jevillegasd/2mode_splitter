...
for i = 1:length(circuits)    
    ...
    for j = 1:length(data)
        ...
        leg_entries = [leg_entries, [num2str(round(v*i*1e3,2)), ' W/m']];
        hold on; grid on;
    end
end
...
legend(leg_entries)
xlim([1520, 1580])
format_plot(gca,'Wavelength (nm)','Insertion Loss (dB)', 5,'1c_double')
...
xlim([1520, 1580])
ylim([-30, 0])
%title(tcl,'Mode Modulator using Adjoint Optimized Y-Splitters. TE1 Input - 250 um Heaters.')
format_plot(gca,'Wavelength (nm)','Insertion Loss (dB)', 5,'1c_double')