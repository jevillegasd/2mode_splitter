...
die = "jvillegas";
circuits = ["TE1_topo_DEV04","TE1_topo_DEV04"];
...
i_mult = 1/2; % 2 circuits per pair of contacts. 
modes = [1 2];
wav_t = 1550; %%%%%%%%%%%%%%%%%% <------------------------------------------------
figure(2)
...
for i = 1:length(circuits)    
    ...
    for j = 1:length(data)
        ...
        v = [v str2double(data{j}.params.MeasuredVoltageCh2)];
        ic = [ic str2double(data{j}.params.CurrentCh2)];
        hold on; grid on;
    end
end
ic = ic*i_mult;
...
for mode = 1:2
    ...
    for v_idx = 1:length(p)
        [pks,locs] = findresonances(-P(mode,:,v_idx),"MinPeakProminence",10);
...
markers = {'o','x'};
plot(p, squeeze(P(1,idx,:)),markers{1},'MarkerSize',4,'LineWidth', 1, 'color',colors(1,:)); hold on;
plot(p, squeeze(P(2,idx,:)),markers{2},'MarkerSize',4,'LineWidth', 1, 'color',colors(2,:));
...
TE0_mf = (TE0)./(TE1+TE0);
TE1_mf = (TE1)./(TE1+TE0);
disp("TE0_mf = "+num2str(10*log10(max(TE0_mf))) + ", TE1" + num2str(10*log10(max(TE1_mf))))

XT_t = (newp_mode(:,1) - newp_mode(:,2));
XT = [-max(XT_t) min(XT_t)];
disp("XT = "+num2str(XT));
...
for mode = modes
    coeff = polyfit(p,dl(mode,:),1);
    plot(p,dl(mode,:),markers{mode},'MarkerSize',4,'LineWidth', 1,'color',colors(mode,:));
    y1 = polyval(coeff,p_);
...
text( x_(1)+abs(x_lim(2)-x_lim(1))/100, y_(1)+abs(y_(2)-y_(1))/10  , "P_\pi = " + num2str(P_pi,3)+" mW", 'FontSize',9)
plot(x_,y_,'--','color','black','LineWidth',1.5);
grid on; hold off; legend(["TE0", "TE1"]);
ylim(y_);
